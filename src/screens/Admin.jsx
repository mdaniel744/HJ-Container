...
function useEntityList(key, queryFn) {
  return useQuery({ queryKey: [key], queryFn }).data || [];
}
...
export default function Admin() {
  ...
  const products = useEntityList("admin-products", () => base44.entities.Product.list("sort_order", 200));
  const variants = useEntityList("admin-variants", () => base44.entities.Variant.list("sku", 500));
  const orders = useEntityList("admin-orders", () => base44.entities.Order.list("-created_date", 200));
  const quotes = useEntityList("admin-quotes", () => base44.entities.QuoteRequest.list("-created_date", 200));
  const messages = useEntityList("admin-messages", () => base44.entities.ContactMessage.list("-created_date", 200));
  const faqs = useEntityList("faqs", () => base44.entities.Faq.list("sort_order", 200));
  const policies = useEntityList("policies", () => base44.entities.PolicyPage.list("sort_order", 60));
  const settingsList = useEntityList("settings-all", () => base44.entities.SiteSetting.list("-created_date", 1));
