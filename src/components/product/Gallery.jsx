  ...
  return (
    <div>
      <div className="relative border border-slate-200 bg-slate-50 overflow-hidden group">
        <Image src={list[active]} alt={alt} className="w-full aspect-[4/3] transition-transform duration-500 group-hover:scale-[1.08]" />
        <button
      ...
      {list.length > 1 && (
        <div className="mt-3 grid grid-cols-4 gap-3">
          {list.map((src, i) => (
            ...
            >
              <Image src={src} alt={`${alt} — ${i + 1}`} className="w-full aspect-square" />
            </button>
          ))}
        </div>
      )}
      ...
      {full && (
        <div className="fixed inset-0 z-[80] bg-slate-950/95 flex items-center justify-center p-6" role="dialog" aria-modal="true">
          ...
          <Image src={list[active]} alt={alt} fittingType="fit" className="max-h-[85vh] w-full max-w-5xl" />
        </div>
      )}
    </div>
  );
